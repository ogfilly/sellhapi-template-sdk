...
// ── Capability storefront types ───────────────────────────────────────────────
// These are the canonical shapes returned by capability storefront endpoints.
// Single source of truth — sell_store and any consumer should import from here.
...
// ── Page context — discriminated union for current route ──────────────────────
// Switch on `page.type` to render the appropriate layout.

export type PageContext =
  | { type: "home" }
  | { type: "category";     slug: string; name: string; image: string | null; products: StoreProduct[]; pagination: StorePagination | null; }
  | { type: "all-products"; products: StoreProduct[]; pagination: StorePagination | null; }
  | { type: "sale";         products: StoreProduct[]; pagination: StorePagination | null; }
  | { type: "collection";   collection: StoreCollection; products: StoreProduct[]; }
  | { type: "section";      section: StoreSection; }
  | { type: "product";      product: StoreProductDetail; }
  // ── Capability page contexts — SDK 0.5.0 ────────────────────────────────────
  // Templates may render these or fall back to a generic layout.
  | { type: "event-list";     events:    StorefrontEvent[];   }
  | { type: "event-detail";   event:     StorefrontEvent;     }
  | { type: "campaign";       campaign:  StorefrontCampaign;  }
  | { type: "service-list";   services:  StorefrontService[]; }
  | { type: "service-detail"; service:   StorefrontService;   };

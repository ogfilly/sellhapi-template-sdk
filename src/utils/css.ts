...
const FONT_FAMILY_MAP: Record<StorefrontConfig["fontStyle"], string> = {
...
const FONT_SCALE_MAP: Record<
  StorefrontConfig["fontScale"],
  { base: string; h1: string; h2: string; h3: string; small: string }
> = {
...
const SPACING_MAP: Record<
  StorefrontConfig["spacing"],
  { section: string; card: string; gap: string }
> = {
...
const RADIUS_MAP: Record<StorefrontConfig["borderRadius"], string> = {
  sharp:   "0px",
  rounded: "10px",
  pill:    "999px",
};

const BUTTON_RADIUS_MAP: Record<StorefrontConfig["buttonShape"], string> = {
  sharp:   "4px",
  rounded: "10px",
  pill:    "999px",
};
...
export function buildCssVariables(config: StorefrontConfig): string {
  ...
  const vars: Record<string, string> = {
    ...
    "--motion-duration":
      config.motionIntensity === "none"       ? "0ms"   :
      config.motionIntensity === "subtle"     ? "200ms" : "400ms",

    "--motion-easing":
      config.motionIntensity === "expressive"
...
export function getCssVariableStyle(
